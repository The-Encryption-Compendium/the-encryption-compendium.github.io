...
<div class="search_result">
  ...
  <hr>
</div>`;Vue.component("search-stats",{props:["n_results","qtime"],template:_search_stats_template,});Vue.component("search-result",{props:["title","published","authors","id"],template:_search_result_template,});function generate_result_component(entry){const el=document.createElement("search-result");el.setAttribute("title",entry.title);el.setAttribute("published",get_publication_date(entry));el.setAttribute("authors",get_authors(entry));el.setAttribute("id",entry.id);return el;}
function filter_by_tags(tags,entries){return entries.filter((entry)=>tags.every((tag)=>"tags"in entry&&entry.tags.includes(tag)));}
...
search(query,tags,entries);document.getElementById("id_query").value=query;document.getElementById("id_query").onkeyup=function(e){input=document.getElementById("id_query").value;console.log(input);document.querySelectorAll(".search_result").forEach((el)=>el.remove());search(input,tags,entries);};